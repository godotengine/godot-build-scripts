...
export ZIP="7z a -r -tzip -mx9 -bso0 -bd"

can_sign_windows=0
if [ ! -z "${WINDOWS_SIGN_NAME}" ] && [ ! -z "${WINDOWS_SIGN_URL}" ] && [ ! -z "${WINDOWS_SIGN_CA}" ] && [[ $(type -P "osslsigncode") ]]; then
  can_sign_windows=1
else
  echo "Disabling Windows binary signing as config.sh does not define the required data (WINDOWS_SIGN_NAME, WINDOWS_SIGN_URL, WINDOWS_SIGN_CA), or osslsigncode can't be found in PATH."
fi
...
sign_windows() {
  ...
  P11_KIT_SERVER_ADDRESS=unix:path=/run/p11-kit/p11kit.sock osslsigncode sign -pkcs11module /usr/lib64/pkcs11/p11-kit-client.so -pkcs11cert 'pkcs11:model=SimplySign%20C' -key 'pkcs11:model=SimplySign%20C' -t http://time.certum.pl/ -ac "${WINDOWS_SIGN_CA}" -n "${WINDOWS_SIGN_NAME}" -i "${WINDOWS_SIGN_URL}" -in $1 -out $1-signed
  mv $1-signed $1
}
...
if [ "${build_classical}" == "1" ]; then
  ...
  $ZIP "${reldir}/${binname}.zip" Godot.app
  rm -rf Godot.app
  ...
  $ZIP "${templatesdir}/macos.zip" macos_template.app
  rm -rf macos_template.app
  ...
  pushd ${templatesdir}/..
  $ZIP "${reldir}/${godot_basename}_export_templates.tpz" templates/*
  popd
...
if [ "${build_mono}" == "1" ]; then
  ...
  $ZIP "${reldir_mono}/${binbasename}_x86_64.zip" ${binbasename}_x86_64
  ...
  $ZIP "${reldir_mono}/${binbasename}_x86_32.zip" ${binbasename}_x86_32
  ...
  $ZIP "${reldir_mono}/${binname}.zip" Godot_mono.app
  rm -rf Godot_mono.app
  ...
  chmod +x macos_template.app/Contents/MacOS/godot_macos*
  sign_macos_template macos_template.app
  $ZIP "${templatesdir_mono}/macos.zip" macos_template.app
  rm -rf macos_template.app
  ...
  $ZIP "${reldir_mono}/${godot_basename}_mono_export_templates.tpz" templates/*
  popd
